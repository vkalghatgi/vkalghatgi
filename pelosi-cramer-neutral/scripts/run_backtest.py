...
def main(placebo_n: int, boot_n: int) -> None:
    ...
    gross0 = run_backtest(tx, sig, md, DEFAULT_PARAMS.with_(cost_bps=0.0, borrow_bps_annual=0.0), JOINT_START, JOINT_END)
    legs = pd.DataFrame(
        {
            name: {
    ...
    for name, df in [("placebo_pelosi_timeshift", pl_shift), ("placebo_pelosi_random", pl_rand_p), ("placebo_cramer_random", pl_rand_c)]:
    ...
    figs["placebo"] = R.fig_placebo(
        s0["sharpe"],
        {"Pelosi time-shift": pl_shift["sharpe"], "Pelosi random tickers": pl_rand_p["sharpe"], "Cramer random tickers": pl_rand_c["sharpe"]},
        fig_dir / "placebo.png",
    )
